# a game in which you have to guess what is higher or lower
data = [
    {
        "name": "Instagram",
        "follower_count": 346,
...
def checking(guess1, follower_a, follower_b):
...
while end_of_program == False:
    ...
    guess = input("Which is Higher?")
    follower_a = account_a["follower_count"]
    follower_b = account_b["follower_count"]
    is_correct = checking(guess, follower_a, follower_b)
    if is_correct:
        score += 1
        print(f"You are right!! Your score is {score} pts.")
        account_a = account_b
        account_b = get_account()
